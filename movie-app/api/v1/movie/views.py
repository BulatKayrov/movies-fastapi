...
def delete_one_movie(background_task: BackgroundTasks, movie=Depends(find_movie_by_slug)):
    background_task.add_task(storage.save())
...
def update_one_movie(movie_in: SMovieUpdate, background_task: BackgroundTasks, movie=Depends(find_movie_by_slug)):
    background_task.add_task(storage.save())
...
def partial_update(
    movie_in: SMoviePartialUpdate, background_task: BackgroundTasks, movie=Depends(find_movie_by_slug)
):
    background_task.add_task(storage.save())
    logger.info('Movie updated')
